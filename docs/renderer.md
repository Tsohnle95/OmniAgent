...
structured progress, and calls with a start timestamp show an elapsed clock.
...
- **Runtime-neutral transcript presentation** — `OpenCodeTimeline.tsx` retains
  ...
  structured progress is reduced to a readable phrase; live command output and
  failures open their bounded disclosure automatically while settled historical
  calls stay compact. Bottom-follow uses a stable signature to follow
