...
Agent TUI mode starts the active runtime command through `agentTuiStart` in a
separate Kitty window with the panel's workspace directory. OpenCode uses
`opencode2 --session <session-id>`, so Kitty's configured font, transparency,
blur, and theme are applied; the DeepSeek profile remains unavailable until
its TUI command is supported.
