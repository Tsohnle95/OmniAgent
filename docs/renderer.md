...
OmniAgent does not depend on), edit-back-to-composer, send-now, and remove.
...
is captured and resolved like OpenChamber, but OmniAgent's prompt API is
...
OmniAgent equivalent and is wired as a no-op parameter.
...
| Component | File | Role |
|---|---|---|
| `App` | `App.tsx` | Layout: titlebar + fixed 3-track grid (sidebar, divider, workspace area) + optional bottom tray. The workspace area is a relative flex row whose editor fills the whole region except the visible strip left of the panels (a computed `--editor-right` margin on the editor pane, so the editor's empty state and content center in the free space between the file tray and the leftmost agent panel, live-tracking every drag and tray change); each open session is an absolutely positioned `agent-col` tray with its own `{left, width, open}` slot state, so no panel can move another. The original (first) panel is right-anchored: it starts at the area's right edge and stays there, resized from its left edge handle. Newly added panels stack 280px wide against the left of the right anchor. Every panel resizes from either edge — dragging the left edge moves it while the right edge stays put, dragging the right edge moves it while the left edge stays put — and only the header drag slides the whole tray (width unchanged) through the file-area space; slides and resizes are clamped by the nearest panels on either side (neighbors sorted by position), stopping at their edges so a drag never alters another panel and panels never overlap. Collapsed panels become 44px trays (rightmost) or slivers. One clamp effect resolves overflow only on window/sidebar resizes — never during a drag — shrinking widths to fit and re-anchoring everything rightmost-first without overlap; the right-anchored agent's width only changes when space is actually squeezed (window shrink or tray open), never when space grows, so closing the file tray or enlarging the window leaves it untouched. The agent-mode titlebar toggle collapses the file tray and splits every open agent panel evenly across the whole workspace area (collapsed model panels reopen into the split); while agent mode is on, panels are free — even the original agent can be slid by its header and resized from either edge and stays where it is put, and manually opening the file tray keeps every panel in place (only widths shrink to fit), since the mode only exits through the toggle itself. Toggling again restores the file tray and every open model shrinks evenly to make room, while a single open agent returns to its default right-anchored width. Clicking a panel focuses it (its editor/tree/terminal state swaps in); titlebar shows a sessions toggle that opens the sidebar on its Sessions pane (expanding the collapsed tray if needed), open-another-workspace buttons, the agent-mode toggle, and the terminal toggle. The model-mode `+` control opens the native folder picker (`selectAddPanel`) so each new panel can be pointed at a different project; when a panel swaps to another directory its stored `{left, width, top, height}` slot migrates to the new session, so the panel keeps its position instead of jumping to a fresh default |
| `Welcome` | `Welcome.tsx` | Editorial two-column OmniAgent launcher: the shared `OmniMark` open-lid terminal SVG, serif wordmark (bundled Cormorant Garamond), folder pick (`selectFolder()`), and a hairline-bordered frame with Sessions/Projects tabs populated from `sessions()` / `projects()`; the landing surface is draggable on macOS while buttons, tabs, and rows remain interactive; both tab lists stay mounted as stacked grid panes (`visibility: hidden` when inactive) so the frame height never changes on tab switch; session rows reopen via `openSessionById` |
| `FileSidebar` | `FileSidebar.tsx` | Tabbed left panel with two top tabs — **Sessions** (`SessionsPane`) and **Files** (CHANGES panel for observed workspace file changes — known baselines open as diffs; unknown baselines are labeled observed — plus the EXPLORER tree, create/rename/delete actions, and drag-and-drop moves onto folders or the root; the EXPLORER section header shows New File / New Folder actions on hover so a file or folder can be created at the workspace root without hovering a specific row; each newly focused session ensure-opens the explorer root via `ensureRootOpen`); a footer cog pinned bottom-left opens the dedicated Settings page in every sidebar state (expanded tabs, single-file mode, collapsed tray) |
| `SettingsPage` | `SettingsPage.tsx` | Dedicated workspace for appearance and agent defaults. Appearance offers persisted Paper Editorial and Original color profiles through `ThemeProvider`; agent behavior owns permissions ask/approve, word wrap, and follow-up queue/steer controls previously shown in the sidebar popover |
| `SessionsPane` | `SessionsPane.tsx` | Sessions tab content styled as a session browser: a New Session action (fresh `openSession` on the current workspace) beside a Plugins menu (workspace commands + skills from `commands()`, run via `runCommand`), a search field filtering every section below it, then Pinned (session ids persisted in `localStorage` "openshell.pinnedSessions", star/unstar from any row), Sessions (collapsible per-workspace dropdowns from `projects()` listing that workspace's sessions, each with its own new-session button), and Recents (all known sessions newest-first). Every expanded section has an independent six-row scroll viewport while the outer pane remains scrollable on short windows. Rows show live/busy dots for open panels and either focus them or reopen closed sessions; rows expose pin and close actions |
