...
| `SessionsTab` | `SessionsTab.tsx` | Sessions rail: running panels with green live status dots (focus/close), recent sessions grouped by workspace directory, and saved workspaces (`projects()` → new session); the Recent sessions and Saved workspaces sections are collapsible dropdowns styled like the sidebar's CHANGES/EXPLORER panels (chevron toggles with counts), and an "Open another workspace" spawn affordance |
