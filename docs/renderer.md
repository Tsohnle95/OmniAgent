...
| `sessionUsage` | `SessionUsage \| null` | active-session projection of cumulative token usage/cost; hydrated from `session.get` whenever a panel is attached (reopen, add control, workspace swap, folder pick, startup restore) and kept fresh by `session.usage.updated` / `session.usage.recorded`; shown in the agent header popup |
...
available area; the empty panel keeps the normal 280px minimum while it is
resized and can still be hidden or expanded with Agent Mode. The terminal
shows a no-workspace notice, and the empty explorer uses
the smaller **Open a workspace** action.

| Component | File | Role |
|---|---|---|
| `App` | `App.tsx` | Layout: titlebar + fixed 3-track grid (sidebar, divider, workspace area) + optional bottom tray. The workspace area is a relative flex row whose editor fills the whole region except the visible strip left of the panels (a computed `--editor-right` margin on the editor pane, so the editor's empty state and content center in the free space between the file tray and the leftmost agent panel, live-tracking every drag and tray change); each open session is an absolutely positioned `agent-col` tray with its own `{left, width, open}` slot state, so no panel can move another. Pointer-move geometry previews update the panel, editor inset, or sidebar grid styles directly and commit React state once on release, keeping large transcript reconciliation off the live resize and slide path; the memoized timeline boundary also skips that release render when its transcript inputs are unchanged. The original (first) panel is right-anchored: it starts at the area's right edge and stays there, resized from its left edge handle. Newly added panels stack 280px wide against the left of the right anchor. Every panel resizes from either edge — dragging the left edge moves it while the right edge stays put, dragging the right edge moves it while the left edge stays put — and only the header drag slides the whole tray (width unchanged) through the file-area space; slides and resizes are clamped by the nearest panels on either side (neighbors sorted by position), stopping at their edges so a drag never alters another panel and panels never overlap. Collapsed panels become 44px trays (rightmost) or slivers. One clamp effect resolves overflow only on window/sidebar resizes — never during a drag — shrinking widths to fit and re-anchoring everything rightmost-first without overlap; the right-anchored agent's width only changes when space is actually squeezed (window shrink or tray open), never when space grows, so closing the file tray or enlarging the window leaves it untouched. The agent-mode titlebar toggle collapses the sidebar and splits every open agent panel evenly across all remaining workspace space, flush against the compact tray (collapsed model panels reopen into the split). Clicking the toggle again restores the prior sidebar state and returns a single agent to its minimum width; a manual panel drag or resize keeps agent-mode geometry stable throughout the pointer gesture, then exits the mode on pointer-up without restoration. Closing the only visible agent from its header hides the tray without closing its session, so the titlebar panel toggle can reopen it; with multiple visible agents, closing a header removes only that panel's session. Clicking a panel focuses it (its editor/tree/terminal state swaps in); titlebar actions are limited to agent mode, terminal, and rightmost Settings because session and workspace opening live in the Sessions pane. The model-mode `+` control opens the native folder picker (`selectAddPanel`) so each new panel can be pointed at a different project; when a panel swaps to another directory its stored `{left, width, top, height}` slot migrates to the new session, so the panel keeps its position instead of jumping to a fresh default |
| `Welcome` | `Welcome.tsx` | Full-window landing view styled verbatim after the T09 "Twin Desk II · Glass Edge" mock: left hero column (orbit mark SVG, serif wordmark, tagline, single **Open a folder** CTA via `selectFolder()`), right accent-washed glass panel with a collapsible **Recent** section (numbered session rows reopening via `reopenSession`) and a **Workspaces** section sourced only from Orbit's saved workspace bookmarks (`openSession(dir)` on rows, latest-activity timestamps); the surface is draggable on macOS while buttons and rows stay interactive, and dropped files open via `openFileWorkspace` |
| `FileSidebar` | `FileSidebar.tsx` | Tabbed left panel with two top tabs — **Sessions** (`SessionsPane`) and **Files** (CHANGES panel for observed workspace file changes — known baselines open as diffs; unknown baselines are labeled observed — plus the EXPLORER tree, rename/delete actions, **Open in Integrated Terminal** for a folder or file parent, and drag-and-drop moves onto folders or the root; the EXPLORER section header shows a New Folder action on hover so a folder can be created at the workspace root without hovering a specific row; each newly focused session ensure-opens the explorer root via `ensureRootOpen`). Workspace and file opening controls live in their task-specific surfaces rather than a redundant icon above the tabs |
...
| `SessionsPane` | `SessionsPane.tsx` | Sessions tab content styled as a session browser with **Open now**, **Workspaces**, and **History** collapsed by default. **New Session** opens the native folder picker without an adjacent file icon. **Open now** merges visible panels with the store's periodically reconciled backend active-session inventory, so detached running sessions remain visible and reopenable. **Workspaces** is Orbit-owned: its hover `+` chooses a folder through `selectDirectory()` and persists the bookmark, each bookmark expands to matching history, and its right-click **Remove from Orbit** action removes only the bookmark. **History** contains known sessions not in Open now. Session rows expose pin and applicable close actions |
...
| `AgentPanel` | `AgentPanel.tsx` | `session`-parameterized panel hosting the runtime-neutral chronological timeline and prompt dock, with a GUI/TUI mode pill in the header. GUI provides the normal timeline and composer; TUI mounts `AgentTui` and runs the active runtime's PTY-backed terminal in the same panel. The GUI timeline includes todo checklist, exact web placeholder, drag-and-drop image attachments, agent/model/variant menus, compact send/stop button, session usage toggle, and smart auto-scroll. Queue changes participate in the bottom-follow signature so adding a queued-message chip cannot leave a bottom-following transcript scrolled upward. Active reasoning, tools, and response text appear only in their source-ordered timeline positions; before the first concrete event arrives, the timeline shows a minimal inline working placeholder instead of duplicating activity above the composer. The composer resolves `/` into a slash-command picker (built-ins like `/compact` first, then `command.list` + `skill.list`; skills run via `session.skill`, `/compact` via `session.compact`) that runs via `runCommand` (Enter on a leading-`/` prompt runs it too) and `@` into a file-mention picker (`file.find` search, debounced per keystroke) that inserts `@rel` tokens attached to the prompt as `PromptFile`s with mention spans. The responsive header renders the workspace once as its picker, with one idle/working indicator that becomes the panel-close `×` on hover, plus a live activity phrase; narrow panels hide the timer and then status phrase before clipping identity or controls. Child sessions replace the workspace picker with its title and expose a back arrow that reopens the parent. The session usage toggle sits in the composer action area, vertically centered over the input, and opens the session/provider usage details; its ring tracks context-window fill |
| `AgentTui` | `AgentTui.tsx` | xterm.js view for a runtime TUI PTY; sends keyboard input and bounded resize dimensions through the preload bridge, filters terminal messages by its generated terminal id, and returns to GUI after natural exit or startup failure |
| `OpenCodeTimeline` | `OpenCodeTimeline.tsx` | Orbit's theme-native runtime-neutral chronological stream: reasoning and tool calls share one quiet work-log grammar with a rail and state markers, while final assistant prose stays separate. Every assistant step remains a stable keyed entry in source order, and only a reasoning block that is the active node's true stream tail receives the running treatment. Running Thinking summaries follow the latest native line and settled Thought summaries return to the first line; summaries wrap to two lines and expanded reasoning uses the assistant Markdown renderer. One-shot summaries remain one-shot summaries, and native text/reasoning deltas render directly without a client-side typewriter queue. A minimal inline working placeholder occupies the next chronological position only until a concrete reasoning, tool, or text event arrives. OpenCode text parts marked `state.phase = "commentary"` normalize into reasoning blocks, while `final_answer` parts remain ordinary assistant output. Text/tool deltas project immediately with narrow-panel-safe Markdown, tools, permission actions, and composer controls. Both OpenCode and DeepSeek feed the same ordered store; DeepSeek native call/result views enrich the shared cards without importing provider colors. Contiguous completed read/list/glob/grep/inspect calls collapse into one expandable exploration summary while every underlying call remains individually inspectable; active and failed calls remain prominent standalone rows. Tool rows expose meaningful running/failure/completion/duration state, suppress sub-second completion noise, auto-open live command output, and keep failed detail collapsed while surfacing the error inline. Generic tool names are inferred from known command/path/query/URL argument shapes, including partial streamed JSON, and truly unidentified calls explicitly say that details are pending. Read/edit tool links normalize an absolute path inside the owning session directory to the workspace-relative path required by the file API and address that session's workspace directly. Edit summaries keep the path, file count, additions, and deletions in one bounded metadata cluster, while long commands and paths truncate with their full value available as a tooltip. Unified diffs derive addition, deletion, and hunk tints from theme tokens rather than fixed dark-theme colors. Projection reconciles stale live/snapshot records sharing a tool call id into one enriched row, preserving a completed live result while accepting an authoritative history name so settled reads and searches never remain labeled as generic tools. Repeated tool snapshots and synthetic dispatch records resolving to the same child session collapse into one stable capped-width delegated-agent card; completion/error synthetic records also settle the retained tool card so its spinner and `Working` label cannot outlive the child run. Its grid keeps the role, task, clamped description, and working/completed/failed state legible without becoming a full-width toolbar, reducing the status to its dot only at the narrowest container size. Opening the card and returning through `AgentPanel` preserve the exact panel slot: pending replacement notifications cannot temporarily attach a second tray, and session-lineage slot memory carries geometry across fresh workspace identities. Interleaved shell, compaction, synthetic, skill, status, and divider rows retain their source order |
...
Collapsed Files/Sessions and agent panels render at 44px. Their collapsed agent
resize targets are wider than normal panel edges, and after 20px of outward drag
