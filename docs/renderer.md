...
| `SessionsPane` | `SessionsPane.tsx` | Sessions tab content styled as a session browser: a New Session action (fresh `openSession` on the current workspace) beside a Plugins menu (workspace commands + skills from `commands()`, run via `runCommand`), a search field filtering every section below it, then Pinned (session ids persisted in `localStorage` "openshell.pinnedSessions", star/unstar from any row), Projects (collapsible per-project dropdowns from `projects()` listing that project's sessions, each with its own new-session button), and Recents (all known sessions newest-first). Rows show live/busy dots for open panels and either focus them or reopen closed sessions; rows expose pin and close actions |
