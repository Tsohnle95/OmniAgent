...
| Component | File | Role |
|---|---|---|
| `App` | `App.tsx` | Layout: titlebar + 3-pane grid (`useDragResize`; `minmax(0,1fr)` center so panels never overflow) + optional bottom tray; left panel collapses to a 44px activity bar, right panel to a 44px model strip; dragging a divider past the window edge collapses that panel on mouse release (width snaps back to its pre-drag value); titlebar shows a tray toggle and busy/idle status; word-wrap shortcuts (⌘W intercepted in main, ⌥Z via `e.code`); darwin class for the traffic-light inset |
...
| `AgentTray` | `AgentTray.tsx` | Shown when the agent panel is collapsed: 44px strip mirroring the left activity bar, with a busy dot and a model-name button (vertical label) that expands the panel back |
