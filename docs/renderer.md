...
| `Welcome` | `Welcome.tsx` | Editorial two-column Orbit launcher: the shared `OrbitMark` open-lid terminal SVG, serif wordmark (bundled Cormorant Garamond), folder pick (`selectFolder()`), and a hairline-bordered frame with Sessions/Projects tabs populated from `sessions()` / `projects()`; the landing surface is draggable on macOS while buttons, tabs, and rows remain interactive; both tab lists stay mounted as stacked grid panes (`visibility: hidden` when inactive) so the frame height never changes on tab switch; session rows reopen via `openSessionById` |
