...
| `OpenCodeTimeline` | `OpenCodeTimeline.tsx` | Theme-native chronological agent stream: visible active reasoning, auto-expanded live exploration, immediately projected text/tool deltas, wrapping narrow-panel content, and compact OmniAgent-accented delegated-agent cards instead of full-width dark or per-agent colored surfaces. Tool and synthetic dispatch records with the same explicit child session ID collapse into one card; it identifies the delegated role and task, shows a pulsing working/completed/failed indicator, and labels the child-session action. Opening that action and returning through `AgentPanel` preserve the exact panel slot: pending replacement notifications cannot temporarily attach a second tray, and session-lineage slot memory carries geometry across fresh workspace identities. Interleaved shell, compaction, synthetic, skill, status, and divider rows retain their source order |
