...
| `OpenCodeTimeline` | `OpenCodeTimeline.tsx` | Theme-native chronological agent stream: visible active reasoning, auto-expanded live exploration, immediately projected text/tool deltas, wrapping narrow-panel content, and compact accent-tinted subagent cards instead of full-width dark surfaces. Subagent cards resolve explicit child session IDs first, show working/completed/failed state, open the child transcript on click, and pair with `AgentPanel` parent navigation; interleaved shell, compaction, synthetic, skill, status, and divider rows retain their source order |
