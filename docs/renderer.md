...
| `FileSidebar` | `FileSidebar.tsx` | CHANGES panel for observed workspace file changes (known baselines open as diffs; unknown baselines are labeled observed), plus the EXPLORER tree, create/rename/delete actions, and drag-and-drop moves onto folders or the root; the EXPLORER section header shows New File / New Folder actions on hover (`section-actions`) so a file or folder can be created at the workspace root without hovering a specific row; each newly focused session ensure-opens the explorer root (`ensureRootOpen` expands and refreshes it without ever collapsing a previously visited workspace) |
...
| `SessionsTab` | `SessionsTab.tsx` | Sessions rail: running panels with green live status dots (focus/close), recent sessions grouped by workspace directory, and saved workspaces (`projects()` → replacement session); the Recent sessions and Saved workspaces sections are collapsible dropdowns styled like the sidebar's CHANGES/EXPLORER panels (chevron toggles with counts) |
