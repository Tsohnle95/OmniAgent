...
| `SessionsPane` | `SessionsPane.tsx` | Sessions tab content styled as a session browser: **New Session** opens the native folder picker without an adjacent file icon. **Open now** merges visible panels with the store's periodically reconciled backend active-session inventory, so detached running sessions remain visible and reopenable. **Workspaces** is Orbit-owned: its hover `+` chooses a folder through `selectDirectory()` and persists the bookmark, each bookmark expands to matching history, and its right-click **Remove from Orbit** action removes only the bookmark. **History** contains known sessions not in Open now. Session rows expose pin and applicable close actions |
