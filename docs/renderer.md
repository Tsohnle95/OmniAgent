...
| `FileSidebar` | `FileSidebar.tsx` | Anthropic-style editorial chrome (scoped warm palette + clay accent + hairline borders + pill hovers, glowing clay dot beside the workspace title): CHANGES panel (agent-touched files, click → diff, always present above the explorer section) as a drag-resizable bottom section with folder context in rows, + EXPLORER tree with a custom line-art SVG icon set (`FileIcons.tsx`: closed/open folder pair in warm sand/clay, document glyphs per type — code/json/markdown/shell/text/config/media/binary — in warm dim, clay when active) and clay-tinted indent guide lines (`.tree-children` wrappers); rows show hover actions (New File / New Folder / ellipsis menu, via `RowActions`) and a right-click context menu (New File / New Folder / Rename / Delete) all with matching mini-icons, inline name inputs (`.tree-input`), Enter commits / Esc cancels; section headers use a rotating custom chevron; header arrow collapses it to an activity bar whose single Explorer button shows the `ShellMark` logo, and whose divider can drag it open |
...
| `AgentPanel` | `AgentPanel.tsx` | Hosts the OpenCode timeline and V2 prompt dock: todo checklist, exact web placeholder, attachment picker, approval toggle, agent/model/variant menus, voice input, compact send/stop button, and smart auto-scroll; header arrow collapses it at the same time as the resize gesture reaches the model strip width; a coin-token toggle in the header opens a usage popup (session tokens/cost from `sessionUsage`, per-provider plan limits from `providerUsage`) |
