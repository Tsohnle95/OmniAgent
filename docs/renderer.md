...
| `OpenCodeTimeline` | `OpenCodeTimeline.tsx` | Theme-native runtime-neutral chronological agent stream: active reasoning is labeled Thinking and auto-expanded while adjacent completed reasoning parts consolidate behind one compact Thought pill, with immediately projected text/tool deltas and narrow-panel-safe Markdown, tools, permission actions, and composer controls. Both OpenCode and DeepSeek feed the same ordered store; DeepSeek native call/result views enrich the shared cards without importing provider colors. Every read, search, list, shell, edit, and delegated-agent call remains an individually identifiable row rather than being collapsed behind a generic activity summary. Tool activity uses the active theme's layered surface, accent border, and warm action label instead of bare ledger rows. Edit summaries keep the path, file count, additions, and deletions in one bounded metadata cluster, while long commands and paths truncate with their full value available as a tooltip. Unified diffs derive addition, deletion, and hunk tints from theme tokens rather than fixed dark-theme colors, and unresolved tools never expose arbitrary input fields inline. Projection reconciles stale live/snapshot records sharing a tool call id into one enriched row, so reopened sessions do not duplicate tools. Tool and synthetic dispatch records with the same explicit child session ID collapse into one capped-width delegated-agent card; its grid keeps the role, task, clamped description, and working/completed/failed state legible without becoming a full-width toolbar, reducing the status to its dot only at the narrowest container size. Opening the card and returning through `AgentPanel` preserve the exact panel slot: pending replacement notifications cannot temporarily attach a second tray, and session-lineage slot memory carries geometry across fresh workspace identities. Interleaved shell, compaction, synthetic, skill, status, and divider rows retain their source order |
