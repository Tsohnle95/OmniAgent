...
| `Welcome` | `Welcome.tsx` | Editorial two-column launcher: serif wordmark (bundled Cormorant Garamond), folder pick (`selectFolder()`), and a hairline-bordered frame with Sessions/Projects tabs populated from `sessions()` / `projects()`; session rows reopen via `openSessionById` |
