...
| `currentModel` | `ModelOption \| null` | seeded per session from the active session selection (falling back to `modelDefault()`), cleared on session switches so a new session never displays the previous session's model; live-updated by `session.model.selected`; includes selected `variant`; carries `limit.context` from the model catalog, which the usage popup uses to compute context-window fill |
...
`openRecovery(id)`, `acknowledgeRecovery(id)`. `commitName`/`deleteEntry`/
