...
| `OpenCodeTimeline` | `OpenCodeTimeline.tsx` | Theme-native chronological agent stream: visible active reasoning, auto-expanded live exploration, immediately projected text/tool deltas, wrapping narrow-panel content, and compact OmniAgent-accented subagent cards instead of full-width dark or per-agent colored surfaces. Subagent cards resolve explicit child session IDs first, show working/completed/failed state, open the child transcript on click, and pair with `AgentPanel` parent navigation; the replacement panel inherits the parent's exact layout slot before paint so opening it and returning do not resize the tray. Interleaved shell, compaction, synthetic, skill, status, and divider rows retain their source order |
