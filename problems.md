...
1. The active OmniAgent workspace does not show every directory physically inside its root. Previously imported `advanced-web-concepts` folders can remain on disk but be absent from Explorer.
2. After deleting `advanced-web-concepts` from disk outside OmniAgent, dragging it into the OmniAgent workspace can report `already exists` even when the item is not visible in Explorer.
3. With the OmniAgent root collapsed, dragging `advanced-web-concepts` into the Explorer area can replace the visible OmniAgent workspace instead of adding a second workspace panel.
...
15. Current iteration: filesystem inspection confirmed `advanced-web-concepts` exists under the OmniAgent root while persisted hidden-path state hid it. Persisted removal tracking is removed; Explorer state is now session-only and reloads enumerate the physical workspace again.
...
21. Current iteration: OmniAgent previously displayed the fallback `build` agent without applying it to sessions that had no recorded agent selection; new sessions now synchronize that real OpenCode agent selection.
